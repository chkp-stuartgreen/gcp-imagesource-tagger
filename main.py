...
def process_cspm_entity(cspm_entity):
    ...
    if "sourceImage" in cspm_entity['entity']:
        ...
        asset['source'] = "/".join(cspm_entity['entity']
                                   ['sourceImage'].split('/')[7:])
        asset['type'] = "image"
        asset['name'] = asset['source'].split('/')[-1]
        # Get project name from URL (selflink?)
        asset['asset_proj'] = cspm_entity['entity']['sourceImage'].split(
            '/')[6]
        return asset
    elif "sourceSnapshot" in cspm_entity['entity']:
        asset['source'] = "/".join(cspm_entity['entity']
                                   ['sourceSnapshot'].split('/')[7:])
        asset['type'] = "snapshot"
        asset['asset_proj'] = cspm_entity['entity']['sourceSnapshot'].split(
            '/')[6]
        asset['name'] = asset['source'].split('/')[-1]
        return asset
...
def process_incoming(request):
    ...
    if asset['type'] == "disk":
        g_asset_info = compute.disks().get(
            zone=asset['zone'], project=asset['asset_proj'], disk=asset['name']).execute()
    elif asset['type'] == "image":
        g_asset_info = compute.images().get(
            project=asset['asset_proj'], image=asset['name']).execute()
    elif asset['type'] == "snapshot":
        g_asset_info = compute.snapshots().get(
            project=asset['asset_proj'], snapshot=asset['name']).execute()
    else:
        # Unknown asset type
        exit()
    ...
    while any(key in g_asset_info for key in loop_triggers):
        ...
        if 'sourceDisk' in g_asset_info:
            g_asset_info = compute.disks().get(
                zone=g_asset_parsed['zone'], project=g_asset_parsed['asset_proj'], disk=g_asset_parsed['name']).execute()
            continue
        if 'sourceImage' in g_asset_info:
            ...
            try:
                g_asset_info = compute.images().get(
                    project=g_asset_parsed['asset_proj'], image=g_asset_parsed['name']).execute()
        ...
        if 'sourceSnapshot' in g_asset_info:
            g_asset_info = compute.snapshots().get(
                project=g_asset_parsed['asset_proj'], snapshot=g_asset_parsed['name']).execute()
            continue
    ...
    creation_timestamp_epoch = time.mktime(time.strptime(
        g_asset_info['creationTimestamp'], time_pattern))
    ...
    entity_label_body['labels']['cloudguard_source_image_project'] = g_asset_info['selfLink'].split(
        '/')[6]
    ...
    # Apply tag to disk
    tag = compute.disks().setLabels(
        project=asset['project'], zone=asset['zone'], resource=asset['original_name'], body=entity_label_body).execute()
